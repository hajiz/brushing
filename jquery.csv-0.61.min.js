RegExp.escape=function(s){return s.replace(/[-/\\^$*+?.()|[\]{}]/g,'\\$&')};(function($){$.csvDefaults={separator:',',delimiter:'"',escaper:'"',skip:0,headerLine:1,dataLine:2};$.csvEntry2Array=function(csv,meta){var meta=(meta!==undefined?meta:{});var separator='separator'in meta?meta.separator:$.csvDefaults.separator;var delimiter='delimiter'in meta?meta.delimiter:$.csvDefaults.delimiter;var escaper='escaper'in meta?meta.escaper:$.csvDefaults.escaper;separator=RegExp.escape(separator);delimiter=RegExp.escape(delimiter);escaper=RegExp.escape(escaper);var reValid=/^\s*(?:Y[^YZ]*(?:ZY[^YZ]*)*Y|[^XYZ\s]*(?:\s+[^XYZ\s]+)*)\s*(?:X\s*(?:Y[^YZ]*(?:ZY[^YZ]*)*Y|[^XYZ\s]*(?:\s+[^XYZ\s]+)*)\s*)*$/;reValid=RegExp(reValid.source.replace(/X/g,separator));reValid=RegExp(reValid.source.replace(/Y/g,delimiter));reValid=RegExp(reValid.source.replace(/Z/g,escaper));var reValue=/(?!\s*$)\s*(?:Y([^YZ]*(?:ZY[^YZ]*)*)Y|([^XYZ\s]*(?:\s+[^XYZ\s]+)*))\s*(?:X|$)/g;reValue=RegExp(reValue.source.replace(/X/g,separator),'g');reValue=RegExp(reValue.source.replace(/Y/g,delimiter),'g');reValue=RegExp(reValue.source.replace(/Z/g,escaper),'g');if(!reValid.test(csv)){return null;}
var output=[];csv.replace(reValue,function(m0,m1,m2){if(m1!==undefined){var reDelimiterUnescape=/ED/g;reDelimiterUnescape=RegExp(reDelimiterUnescape.source.replace(/E/,escaper),'g');reDelimiterUnescape=RegExp(reDelimiterUnescape.source.replace(/D/,delimiter),'g');output.push(m1.replace(reDelimiterUnescape,delimiter));}else if(m2!==undefined){output.push(m2);}
return'';});var reEmptyLast=/S\s*$/;reEmptyLast=RegExp(reEmptyLast.source.replace(/S/,separator));if(reEmptyLast.test(csv)){output.push('');}
return output;};$.array2CSVEntry=function(array,meta){var meta=(meta!==undefined?meta:{});var separator='separator'in meta?meta.separator:$.csvDefaults.separator;var delimiter='delimiter'in meta?meta.delimiter:$.csvDefaults.delimiter;var escaper='escaper'in meta?meta.escaper:$.csvDefaults.escaper;var output=[]
